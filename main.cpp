...
bool SendDiagnosticsCmdWithRetry(string action) {
    string binDir = GetBinPath();
    string cmd = binDir + "\\idevicediagnostics.exe " + action;
    
    string out = ExecCmdAndGetOutput(cmd, binDir);
    if (out.find("Password protected") != string::npos) {
        if (action == "restart") {
            HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
            SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY);
            cout << "[!] Password protected! Exploiting recovery mode loophole to reboot..." << endl;
            SetConsoleTextAttribute(hConsole, 7);
            
            if (EnterRecovery()) {
                cout << "[*] Sending reboot command from Recovery Mode..." << endl;
                ExecCmdAndGetOutput(binDir + "\\irecovery.exe -c reboot", binDir);
                return true;
            }
            return false;
        }
        ...
        cout << "[-] ERROR: Device is locked with a passcode." << endl;
        SetConsoleTextAttribute(hConsole, FOREGROUND_GREEN | FOREGROUND_INTENSITY);
        cout << "[!] Please unlock your device screen! Checking continuously..." << endl;
        SetConsoleTextAttribute(hConsole, 7);
        
        bool success = false;
        for (int i = 10; i > 0; i--) {
            cout << "\rChecking... Timeout in " << i << "s (Press 'c' to cancel)   " << flush;
            ...
            out = ExecCmdAndGetOutput(cmd, binDir);
            if (out.find("Password protected") == string::npos && 
                out.find("ERROR") == string::npos && 
                out.find("Could not") == string::npos) {
                success = true;
                break;
            }
        }
        ...
        if (success) {
            ...
            HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
            SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_INTENSITY);
            cout << "[-] Timeout waiting for unlock or error occurred." << endl;
            SetConsoleTextAttribute(hConsole, 7);
            return false;
        }
    } else if (out.find("ERROR") != string::npos || out.find("Could not") != string::npos) {
        HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
        SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_INTENSITY);
        cout << "[-] Error executing command: " << endl << out;
        SetConsoleTextAttribute(hConsole, 7);
        return false;
    }
    return true;
    ...
    while (true) {
        ...
        if (state == "NORMAL") {
            if (choice == "1") {
                ...
                cout << "[*] Sending reboot command..." << endl;
                if (SendDiagnosticsCmdWithRetry("restart")) {
                    cout << "[+] Command sent!" << endl;
                }
                Sleep(2000);
                forceRedraw = true;
            } else if (choice == "4") {
                cout << "[*] Sending shutdown command..." << endl;
                if (SendDiagnosticsCmdWithRetry("shutdown")) {
                    cout << "[+] Command sent!" << endl;
                }
                Sleep(2000);
                forceRedraw = true;
            ...
            if (choice == "1") {
                ...
                cout << "[*] Rebooting device to Normal Mode for shutdown..." << endl;
                string binDir = GetBinPath();
                ExecCmdAndGetOutput(binDir + "\\irecovery.exe -n", binDir);
                ExecCmdAndGetOutput(binDir + "\\irecovery.exe -c reboot", binDir);
                
                cout << "[*] Waiting for device to boot up in Normal Mode..." << endl;
                bool success = false;
                for (int i = 0; i < 60; i++) {
                    string currState; int currCpid;
                    GetAppleState(currState, currCpid);
                ...
                if (success) {
                    cout << "[*] Device detected! Sending shutdown command..." << endl;
                    if (SendDiagnosticsCmdWithRetry("shutdown")) {
                        cout << "[+] Shutdown command sent successfully!" << endl;
                    }
                } else {
                    cout << "[-] Timeout waiting for Normal Mode. Shutdown failed." << endl;
                }
                cout << "\nPress Enter to return to menu";
                cin.ignore();
                forceRedraw = true;
