...
try {
    ...
    foreach ($asset in $Release.assets) {
        if ($asset.name -eq $ExeName) { $DownloadUrl = $asset.browser_download_url }
        if ($asset.name -eq "hash.txt") { $HashUrl = $asset.browser_download_url }
    }
    ...
    $needsDownload = $true
    if ($HashUrl -and (Test-Path $DestPath)) {
        $remoteHash = (Invoke-RestMethod -Uri $HashUrl -UseBasicParsing).Trim()
        $localHash = (Get-FileHash -Algorithm SHA256 $DestPath).Hash
        if ($remoteHash -eq $localHash) {
            Write-Host "[+] Local version is already up to date! Skipping download." -ForegroundColor Green
            $needsDownload = $false
        }
    }
    ...
    if ($needsDownload) {
        if (Test-Path $DestPath) {
            Write-Host "[*] Found older/different version. Preparing to update..." -ForegroundColor Cyan
